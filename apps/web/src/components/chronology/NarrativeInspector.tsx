      ...
      <div>
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/10 pb-4">
          ...
          <div className="flex items-center gap-1.5">
            <button id="btn-nav-prev-volume" onClick={() => { sounds.playSelect(); onPrevVolume(); }} disabled={!hasPrev} aria-label="Tomo anterior" className={`min-h-[44px] p-1.5 rounded-lg border text-xs font-mono flex items-center gap-1 transition-all ${ hasPrev ? 'border-white/10 bg-surface-container-low text-on-surface hover:bg-surface-container-high hover:text-white' : 'border-on-surface/15 bg-surface-container-lowest text-on-surface-variant/40 cursor-not-allowed' }`} title="Tomo Anterior"> <ChevronLeft className="w-4 h-4" /> <span className="hidden sm:inline">Anterior</span> </button>
            <button id="btn-nav-next-volume" onClick={() => { sounds.playSelect(); onNextVolume(); }} disabled={!hasNext} aria-label="Tomo siguiente" className={`min-h-[44px] p-1.5 rounded-lg border text-xs font-mono flex items-center gap-1 transition-all ${ hasNext ? 'border-white/10 bg-surface-container-low text-on-surface hover:bg-surface-container-high hover:text-white' : 'border-on-surface/15 bg-surface-container-lowest text-on-surface-variant/40 cursor-not-allowed' }`} title="Siguiente Tomo"> <span className="hidden sm:inline">Siguiente</span> <ChevronRight className="w-4 h-4" /> </button>
          </div>
        </div>
