...
export const TimelineHeatmap = React.memo(({ duration, insights, className }: TimelineHeatmapProps) => {
    ...
    return (
        <div className={cn("absolute bottom-full left-0 w-full flex items-end pointer-events-none opacity-20 group-hover:opacity-40 transition duration-slow mb-0.5 z-0", className)} style={{ height: "10px" }}>
            {insights.map((node, i) => {
                ...
                return (
                    <div
                        key={i}
                        className={cn(
                            "flex-1 mx-[0.25px] rounded-t-[0.5px] transition-[height,background-color] duration-base",
