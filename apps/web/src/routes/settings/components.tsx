...
export function ScanButton({ variant, onClick, loading }: { variant: "delta" | "full", onClick: () => void, loading?: boolean }) {
    ...
    return (
        <button
            ...
            disabled={loading}
            className={cn(
                "flex-1 flex items-center justify-between p-6 rounded-container border transition-all duration-500 relative overflow-hidden group/scanbtn",
                isDelta
                    ? "bg-cyan-500/8 border-cyan-500/20 hover:bg-cyan-500/15 hover:border-cyan-500/35 text-cyan-400"
                    : "bg-surface-container border border-outline-variant rounded-container hover:bg-surface-container-high hover:border-outline-variant text-on-surface",
        ...
        >
            ...
            <div className={cn(
                ...
                isDelta
                    ...
                    : "bg-surface-container border-outline-variant/8 group-hover/scanbtn:bg-surface-container-high"
            )}>
                <RefreshIcon
                    className={cn(
                        "transition-all duration-700",
                        isDelta ? "text-cyan-400" : "text-on-surface/80 group-hover/scanbtn:text-on-surface",
    ...
    return (
        <div className={cn(
            ...
            <div className="flex-1 min-w-0">
                ...
                <p className="text-[10px] font-black uppercase tracking-widest text-on-surface-variant mt-0.5">{status}</p>
            </div>
            ...
            {!connected && !disabled && (
                <span className="text-[9px] font-black tracking-widest uppercase text-on-surface-variant border border-outline-variant/50 px-2.5 py-1 rounded-full group-hover/intcard:border-outline-variant group-hover/intcard:text-on-surface-variant transition-colors shrink-0">
                    DISPONIBLE
                </span>
            )}

            {disabled && (
                <span className="text-[9px] font-black tracking-widest uppercase text-on-surface-variant/60 border border-outline-variant/50 px-2.5 py-1 rounded-full shrink-0">
                    PRONTO
                </span>
            )}
        </div>
...
export function StatusCard({ label, value, icon: Icon }: { label: string; value: string; icon: React.ElementType }) {
    return (
        <div className="p-6 bg-surface-container border border-outline-variant rounded-container hover:border-cyan-500/30 flex items-center gap-4 group/statuscard relative overflow-hidden transition-all duration-300">
            ...
            <div className="relative z-10">
                <p className="text-[9px] font-black uppercase tracking-[0.25em] text-on-surface-variant mb-0.5 font-mono">{label}</p>
...
export function Card({ children, className }: { children: React.ReactNode; className?: string }) {
    return (
        <div className={cn("bg-surface-container rounded-container shadow-elevation-1", className)}>
            {children}
        </div>
    )
}
...
export function PathList({ label, directories, onAdd, onRemove, placeholder }: PathListProps) {
    ...
    return (
        <div className="p-6 space-y-4">
                    ...
                    {directories.map((dir) => (
                        <div key={dir} className="flex items-center justify-between bg-surface-container border border-outline-variant rounded-xl px-4 py-2.5">
                            ...
                            <button
                                ...
                                className="text-on-surface-variant hover:text-red-400 transition-colors duration-200 p-1 rounded-lg hover:bg-surface-container-high"
                            >
            ...
            <div className="flex items-center gap-2.5 bg-surface-container border border-outline-variant rounded-xl pl-1 pr-1.5 py-1 focus-within:border-[#ff6e3a]/40 transition-all">
                <input
                    type="text"
                    value={inputValue}
                ...
                <button
                    type="button"
                    onClick={handleAdd}
                    className="bg-[#ff6e3a] hover:bg-[#ff7e4e] text-zinc-950 px-5 py-2.5 rounded-lg font-black uppercase text-[10px] tracking-wider transition-all shrink-0 flex items-center gap-1.5 active:scale-95 shadow-elevation-2"
                >
                    <PlusIcon />
                    AGREGAR
                </button>
    ...
    return (
        <div className="flex flex-col md:flex-row md:items-center justify-between px-6 py-5 border-b border-outline-variant/4 last:border-0 hover:bg-surface-variant/[0.01] transition-all duration-200 gap-5 group/input">
            <div className="space-y-1 flex-1 max-w-xl">
                ...
                {description && <p className="text-[11px] text-on-surface-variant leading-relaxed font-medium group-hover/input:text-on-surface-variant transition-colors duration-300">{description}</p>}
            </div>
            <div className={cn(
                ...
                "focus-within:border-[#ff6e3a]/50 focus-within:shadow-[0_0_24px_rgba(255,110,58,0.15)] focus-within:bg-surface-container-high hover:border-outline-variant/15",
                className
            )}>
                <input
                    ref={ref}
    ...
    return (
        <div className="flex flex-col md:flex-row md:items-center justify-between px-6 py-5 border-b border-outline-variant/4 last:border-0 hover:bg-surface-variant/[0.01] transition-all duration-200 gap-5 group/select">
            <div className="space-y-1 flex-1 max-w-xl">
                ...
                {description && <p className="text-[11px] text-on-surface-variant leading-relaxed font-medium group-hover/select:text-on-surface-variant transition-colors duration-300">{description}</p>}
            </div>
            <div className={cn(
                ...
                <select
                    ref={ref}
                    className="flex-1 bg-transparent text-on-surface text-xs focus:outline-none appearance-none cursor-pointer pr-6 font-medium [&>option]:bg-[#141418] [&>option]:text-on-surface"
                    {...props}
                >
                    {options.map((opt) => (
    ...
    return (
        ...
        >
            <div className="space-y-1 flex-1 max-w-xl">
                ...
                {description && <p className="text-[11px] text-on-surface-variant leading-relaxed font-medium group-hover/toggle:text-on-surface-variant transition-colors duration-300">{description}</p>}
            </div>
            <button
                type="button"
                role="switch"
