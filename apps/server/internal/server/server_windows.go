...
func addQuitItem() {
	...
	mQuit := systray.AddMenuItem("Quit KameHouse", "Quit the whole app")
	mQuit.Enable()
	go func() {
		<-mQuit.ClickedCh
...
func onReady(webFS *embed.FS, app *core.App, flags core.KameHouseFlags, selfupdater *updater.SelfUpdater) func() {
	return func() {
		...
		systray.SetTooltip(fmt.Sprintf("KameHouse v%s", constants.Version))
		log.Trace().Msg("systray: App is ready")

		// Menu items
		systray.AddMenuItem("KameHouse v"+constants.Version, "KameHouse version")
