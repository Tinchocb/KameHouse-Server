...
func NewEpisode(opts *NewEpisodeOptions) *Episode {
	...
	if opts.LocalFile != nil {
		...
		entryEp.SeasonNumber = opts.LocalFile.GetSeasonNumber()
			...
			case LocalFileTypeMain:
				if foundAnimapEpisode {
					...
					if opts.Media.Format == "MOVIE" {
						entryEp.DisplayTitle = opts.Media.TitleEnglish
	...
	if dbEp != nil {
		if dbEp.Title != "" {
			entryEp.EpisodeTitle = dbEp.Title
			entryEp.EpisodeMetadata.Title = dbEp.Title
			if entryEp.DisplayTitle == "" || entryEp.DisplayTitle == "Episode "+strconv.Itoa(entryEp.EpisodeNumber) {
				entryEp.DisplayTitle = dbEp.Title
			}
		}
		if dbEp.Description != "" {
			entryEp.EpisodeMetadata.Overview = dbEp.Description
			entryEp.EpisodeMetadata.Summary = dbEp.Description
		}
		if dbEp.Image != "" {
			entryEp.EpisodeMetadata.Image = dbEp.Image
			entryEp.EpisodeMetadata.HasImage = true
		}
		if dbEp.RuntimeMinutes > 0 {
			entryEp.EpisodeMetadata.Length = dbEp.RuntimeMinutes
		}
...
func NewSimpleEpisode(opts *NewSimpleEpisodeOptions) *Episode {
	...
	if opts.LocalFile != nil {
		...
		entryEp.EpisodeMetadata.Image = opts.Media.PosterImage
