...
func (pd *Playlist) FindEpisode(mId int, episodeNumber int) (*PlaylistEpisode, bool) {
	for _, e := range pd.Episodes {
		if e.Episode.BaseAnime != nil && int(e.Episode.BaseAnime.ID) == mId && e.Episode.EpisodeNumber == episodeNumber {
			return e, true
		}
	}
	return nil, false
...
func (pd *Playlist) SetEpisodeCompleted(mId int, anidbEpisode string) {
	for _, e := range pd.Episodes {
		if e.Episode.BaseAnime != nil && int(e.Episode.BaseAnime.ID) == mId && e.Episode.AniDBEpisode == anidbEpisode {
			e.IsCompleted = true
			return
		}
	}
