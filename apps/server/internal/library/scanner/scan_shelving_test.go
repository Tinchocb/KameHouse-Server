...
func TestScanner_Shelving(t *testing.T) {
	...
	tempDir, err := os.MkdirTemp("", "KameHouse_test_library")
	if err != nil {
		t.Fatal(err)
	}
	...
	t.Run("Shelve missing locked file", func(t *testing.T) {
		...
		lockedLf := dto.NewLocalFile(filePath, tempDir)
		lockedLf.Locked = true
		lockedLf.MediaId = 1

		existingLfs := []*dto.LocalFile{lockedLf}
	...
	t.Run("Unshelve reappearing file", func(t *testing.T) {
		...
		shelvedLf := dto.NewLocalFile(filePath, tempDir)
		shelvedLf.Locked = true

		existingShelvedFiles := []*dto.LocalFile{shelvedLf}
		var existingLfs []*dto.LocalFile
	...
	t.Run("Keep shelved file if library path missing", func(t *testing.T) {
		...
		// Prepare shelved file
		shelvedLf := dto.NewLocalFile(missingFilePath, missingDir)
		shelvedLf.Locked = true
