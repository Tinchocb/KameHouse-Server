...
// resolveAbsoluteByOffset resolves an absolute episode number into a relative episode + media ID
// using only the cumulative episode counts from the media tree (no AniDB metadata required).
//
// This is the Dragon Ball/Naruto/One Piece fallback path used when:
// - AniDB metadata is unavailable for some branches
// - The date-sorted fallback in getRelativeEpisodeNumber fails
//
// Example: Dragon Ball franchise has DB (153 eps), DBZ (291 eps), DBGT (64 eps), DBS (131 eps)
// Input: absolute ep 200 → maps to DBZ ep 47 (200 - 153 = 47)
func (o *MediaTreeAnalysis) resolveAbsoluteByOffset(abs int) (relativeEp int, mediaId int, ok bool) {
	if len(o.branches) == 0 {
		return 0, 0, false
	}

	// Accumulate episode counts across branches in order
	cumulativeOffset := 0
	for _, b := range o.branches {
		branchStart := cumulativeOffset + 1
		branchEnd := cumulativeOffset + b.totalEpisodeCount

		if abs >= branchStart && abs <= branchEnd {
			relativeEp = abs - cumulativeOffset
			return relativeEp, b.media.ID, true
		}

		cumulativeOffset = branchEnd
	}
