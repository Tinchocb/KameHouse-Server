...
func TestMatcher3(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "Gotoubun no Hanayomeï¼Š",
			paths: []string{
				"E:/Anime/Go-toubun no Hanayome/[Shimai] Gotoubun no Hanayomeï¼Š - 01.mkv",
			},
...
func TestMatcherWithOfflineDB(t *testing.T) {
	...
	mf, err := NewMediaFetcher(t.Context(), &MediaFetcherOptions{
		...
		LocalFiles:                 []*dto.LocalFile{}, // Empty, we don't need local files for fetching
...
func TestIgnoredSynonyms(t *testing.T) {
	t.Skip("Skipped: getIgnoredSynonyms was removed from refactored Matcher")
}
...
func TestGetFileFormatType(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			lf := dto.NewLocalFile(tt.path, dir)
