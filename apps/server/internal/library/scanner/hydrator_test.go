...
func TestFileHydrator_HydrateMetadata(t *testing.T) {
	...
	tests := []struct {
		...
		paths           []string
		expectedMediaId int
		expectedType    dto.LocalFileType
		config          string
	}{
		{
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, path := range tt.paths {
				lf := dto.NewLocalFile(path, "E:/Anime")
				lfs = append(lfs, lf)
			}
			...
			for _, nm := range mc.NormalizedMedia {
				t.Logf("media id: %d, title: %s", nm.ID, dto.GetTitleSafe(nm))
			}
...
func TestFileHydrator_applyHydrationRule(t *testing.T) {
	...
	tests := []struct {
		...
		expectedResult bool
		expectedMeta   *dto.LocalFileMetadata
	}{
		{
