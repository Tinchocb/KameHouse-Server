...
func (ad *AutoDownloader) fetchRunData(ctx context.Context, ruleIDs ...uint) (*runData, error) {
	...
	rules, err := db.GetAutoDownloaderRules(ad.database)
	if err != nil {
		return nil, fmt.Errorf("failed to fetch rules: %w", err)
	}

	// Filter out disabled rules
	rules = lo.Filter(rules, func(r *dto.AutoDownloaderRule, _ int) bool {
	...
	profiles, err := db.GetAutoDownloaderProfiles(ad.database)
	if err != nil {
	...
	lfs, _, err := db.GetLocalFiles(ad.database)
	if err != nil {
...
func (ad *AutoDownloader) getRuleProfiles(rule *dto.AutoDownloaderRule, profiles []*dto.AutoDownloaderProfile) []*dto.AutoDownloaderProfile {
	ruleProfiles := make([]*dto.AutoDownloaderProfile, 0)
	for _, profile := range profiles {
		if profile.Global {
...
func (ad *AutoDownloader) calculateCandidateScore(t *NormalizedTorrent, ruleProfiles []*dto.AutoDownloaderProfile) (score int, requiredMinScore int) {
	score = 0
	requiredMinScore = 0
...
func (ad *AutoDownloader) getDelaySettings(rule *dto.AutoDownloaderRule, ruleProfiles []*dto.AutoDownloaderProfile) delaySettings {
...
func (ad *AutoDownloader) selectAndDownloadBestCandidates(isSimulation bool, groupedCandidates map[uint]map[int][]*Candidate, rules []*dto.AutoDownloaderRule, profiles []*dto.AutoDownloaderProfile) int {
	downloaded := 0
	mu := sync.Mutex{}

	for ruleID, episodes := range groupedCandidates {
		rule, found := lo.Find(rules, func(r *dto.AutoDownloaderRule) bool {
...
func (ad *AutoDownloader) queueTorrentForDelay(isSimulation bool, rule *dto.AutoDownloaderRule, episode int, candidate *Candidate, delayMinutes int) {
	if isSimulation {
...
func (ad *AutoDownloader) inheritReleaseGroupsFromProfiles(rule *dto.AutoDownloaderRule, profiles []*dto.AutoDownloaderProfile) []string {
...
func (ad *AutoDownloader) downloadTorrent(isSimulation bool, t *NormalizedTorrent, rule *dto.AutoDownloaderRule, episode int, score int, existingItem *models.AutoDownloaderItem) bool {
...
func (ad *AutoDownloader) isAdditionalTermsMatch(torrentName string, rule *dto.AutoDownloaderRule) (ok bool) {
...
func (ad *AutoDownloader) isTitleMatch(torrentParsedData *habari.Metadata, torrentName string, rule *dto.AutoDownloaderRule, listEntry *anilist.AnimeListEntry) (ok bool) {
	defer util.HandlePanicInModuleThen("autodownloader/isTitleMatch", func() {
		ok = false
	})

	switch rule.TitleComparisonType {
	case dto.AutoDownloaderRuleTitleComparisonContains:
		// +---------------------+
		// |   Title "Contains"  |
		// +---------------------+
	...
	case dto.AutoDownloaderRuleTitleComparisonLikely:
		// +---------------------+
		// |   Title "Likely"    |
		// +---------------------+
		...
		if parsedComparisonTitle.Title != "" && len(parsedComparisonTitle.SeasonNumber) > 0 {
			...
			// e.g. Torrent: "[KameHouse] Jujutsu Kaisen 2nd Season - 20 [...].mkv" -> "Jujutsu Kaisen"
...
func (ad *AutoDownloader) isSeasonAndEpisodeMatch(
	...
	case dto.AutoDownloaderRuleEpisodeRecent:
		// +---------------------+
		// |  Episode "Recent"   |
		// +---------------------+
	...
	case dto.AutoDownloaderRuleEpisodeSelected:
		// +---------------------+
		// | Episode "Selected"  |
		// +---------------------+
...
func (ad *AutoDownloader) getRuleListEntry(rule *dto.AutoDownloaderRule) (*anilist.AnimeListEntry, bool) {
...
func (ad *AutoDownloader) getProvidersForRules(rules []*dto.AutoDownloaderRule, profiles []*dto.AutoDownloaderProfile) []extension.AnimeTorrentProviderExtension {
	...
	for _, rule := range rules {
		...
		if rule.ProfileID != nil {
			profile, found := lo.Find(profiles, func(p *dto.AutoDownloaderProfile) bool {
...
func (ad *AutoDownloader) isProviderMatch(t *NormalizedTorrent, rule *dto.AutoDownloaderRule) bool {
	if len(rule.Providers) == 0 {
		return true
	}
