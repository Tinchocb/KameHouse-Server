...
type (
	Settings struct {
		...
		// SupportsBidirectionalSync indicates if KameHouse can pull data from the tracker
	...
	MediaEntry struct {
		...
		// MediaId is the AniList or custom source media ID (KameHouse ID).
		...
		// ExternalId
		//	When Pulling: The extension MUST populate this.
		//	When Pushing: KameHouse will populate this (using ResolveExternalId).
	...
	Provider interface {
		...
		// ResolveExternalId finds the tracker-specific ID for a given KameHouse media entry.
		// KameHouse calls this BEFORE calling PushEntry to ensure MediaEntry.ExternalId is populated.
...
const (
	...
	DiffTypeLocalOnly    DiffType = "local-only"    // Exists in KameHouse, missing in Tracker
	DiffTypeRemoteOnly   DiffType = "remote-only"   // Exists in Tracker, missing in KameHouse
