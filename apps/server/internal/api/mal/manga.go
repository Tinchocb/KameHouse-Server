...
func (w *Wrapper) GetMangaDetails(mId int) (*BasicManga, error) {
	...
	edge := httpclient.NewEdgeClient[BasicManga]("", 10*time.Second, w.logger)
	edge.SetHeader("Authorization", "Bearer "+w.AccessToken)
	manga, err := edge.Execute(context.Background(), "GET", reqUrl, nil)
	if err != nil {
...
func (w *Wrapper) GetMangaCollection() ([]*MangaListEntry, error) {
	...
	edge := httpclient.NewEdgeClient[response]("", 10*time.Second, w.logger)
	edge.SetHeader("Authorization", "Bearer "+w.AccessToken)
	data, err := edge.Execute(context.Background(), "GET", reqUrl, nil)
	if err != nil {
...
func (w *Wrapper) UpdateMangaListStatus(opts *MangaListStatusParams, mId int) error {
	...
	edge := httpclient.NewEdgeClient[any]("", 10*time.Second, w.logger)
	edge.SetHeader("Authorization", "Bearer "+w.AccessToken)
	edge.SetHeader("Content-Type", "application/x-www-form-urlencoded")
	_, err := edge.Execute(context.Background(), "PATCH", reqUrl+"?"+encodedData, nil)
	if err != nil {
...
func (w *Wrapper) DeleteMangaListItem(mId int) error {
	...
	edge := httpclient.NewEdgeClient[any]("", 10*time.Second, w.logger)
	edge.SetHeader("Authorization", "Bearer "+w.AccessToken)
	_, err := edge.Execute(context.Background(), "DELETE", reqUrl, nil)
	if err != nil {
