...
import (
	...
	discordrpc_client "kamehouse/internal/discordrpc/client"
...
var (
	defaultActivity = discordrpc_client.Activity{
		...
		Assets: &discordrpc_client.Assets{
			LargeImage: "",
			LargeText:  "",
			SmallImage: "https://kamehouse.app/images/circular-logo.png",
			SmallText:  "KameHouse v" + constants.Version,
			SmallURL:   "https://kamehouse.app",
		},
...
func isKameHouseButtonPresent(activity *discordrpc_client.Activity) bool {
	if activity == nil || activity.Buttons == nil {
		return false
	}
	for _, button := range activity.Buttons {
		if button.Label == "KameHouse" && button.Url == "https://kamehouse.app" {
			return true
		}
	}
	return false
...
func (p *Presence) SetAnimeActivity(a *AnimeActivity) {
	...
	if !(p.settings.RichPresenceHideKameHouseRepositoryButton || len(activity.Buttons) > 1) {
		activity.Buttons = append(activity.Buttons, &discordrpc_client.Button{
			Label: "KameHouse",
			Url:   "https://kamehouse.app",
		})
	...
	// Only allow changing small image and text if KameHouse button is present
	if isKameHouseButtonPresent(&activity) {
