...
func FindAutoSelectProfile(db *Database) (*dto.AutoSelectProfile, bool) {
...
func GetAutoSelectProfile(db *Database) (*dto.AutoSelectProfile, error) {
	var res models.AutoSelectProfile
	err := db.Gorm().First(&res).Error
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return &dto.AutoSelectProfile{}, nil
		}
		return nil, err
	}

	// Unmarshal the data
	var profile dto.AutoSelectProfile
...
func SaveAutoSelectProfile(db *Database, profile *dto.AutoSelectProfile) error {
	// Marshal the data
...
func DeleteAutoSelectProfile(db *Database) error {
