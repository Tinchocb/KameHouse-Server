...
func (cw *CollectionWrapper) addAnimeEntry(mediaId int, status anilist.MediaListStatus) error {
	...
	// Fetch media data
	var mediaNode *anilist.BaseAnime

	// Use GetAnime which already has the logic to fetch from custom sources or cache
	mediaResp, err := cw.platform.GetAnime(context.Background(), mediaId)
	if err != nil {
		return err
	}
	mediaNode = mediaResp
...
func (cw *CollectionWrapper) addMangaEntry(mediaId int, status anilist.MediaListStatus) error {
	...
	// Fetch media data
	var mediaNode *anilist.BaseManga

	// Use GetManga which already has the logic to fetch from custom sources or cache
	mediaResp, err := cw.platform.GetManga(context.Background(), mediaId)
	if err != nil {
		return err
	}
	mediaNode = mediaResp
