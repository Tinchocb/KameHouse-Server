...
// videoProxyClient is used exclusively by Nakama streams, which require
// proxying because they inject authentication headers (X-KameHouse-Nakama-Token).
...
func (s *Nakama) GetStreamHandler() http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.Method == http.MethodHead {
			...
			w.Header().Set("X-KameHouse-Nakama-Token", s.nakamaHostPassword)
		...
		req.Header.Set("X-KameHouse-Nakama-Token", s.nakamaHostPassword)
