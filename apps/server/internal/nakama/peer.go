...
// isRoomURL checks if the URL is a KameHouse Rooms URL
...
// attemptDirectConnection makes a direct H2P connection to a KameHouse host
func (m *Manager) attemptDirectConnection(connCtx context.Context) error {
	...
	headers := http.Header{}
	headers.Set("X-KameHouse-Nakama-Token", m.settings.RemoteServerPassword)
	headers.Set("X-KameHouse-Nakama-Username", username)
	headers.Set("X-KameHouse-Nakama-Server-Version", constants.Version)
	headers.Set("X-KameHouse-Nakama-Peer-Id", peerID)
...
// attemptRoomConnection connects to a KameHouse Rooms relay as a peer
func (m *Manager) attemptRoomConnection(connCtx context.Context) error {
	...
	u, err := url.Parse(fmt.Sprintf("%s/%s/peer", constants.KameHouseRoomsApiWsUrl, roomId))
	if err != nil {
		return err
	}
	q := u.Query()
