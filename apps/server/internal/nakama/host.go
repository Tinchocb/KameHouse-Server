...
func (m *Manager) RoomsAvailable() bool {
	resp, err := m.reqClient.R().
		Get(constants.KameHouseRoomsApiUrl + "/health")
	if err != nil {
		return false
	}
	...
	if createResp.Status != "healthy" && createResp.Version != constants.KameHouseRoomsVersion {
		return false
	}
...
// CreateAndJoinRoom creates a room using KameHouse Rooms API and connects as host
...
func (m *Manager) HandlePeerConnection(w http.ResponseWriter, r *http.Request) {
	...
	username := r.Header.Get("X-KameHouse-Nakama-Username")
	...
	peerID := r.Header.Get("X-KameHouse-Nakama-Peer-Id")
	if peerID == "" {
	...
	serverVersion := r.Header.Get("X-KameHouse-Nakama-Server-Version")
