...
//	@returns dto.AutoSelectProfile
func (h *Handler) HandleGetAutoSelectProfile(c echo.Context) error {
	profile, err := db.GetAutoSelectProfile(h.App.Database)
	if err != nil {
		return h.RespondWithError(c, err)
	}
...
func (h *Handler) HandleSaveAutoSelectProfile(c echo.Context) error {
	type body struct {
		Profile *dto.AutoSelectProfile `json:"profile"`
	}
	...
	if err := db.SaveAutoSelectProfile(h.App.Database, b.Profile); err != nil {
		return h.RespondWithError(c, err)
	}
...
func (h *Handler) HandleDeleteAutoSelectProfile(c echo.Context) error {
	if err := db.DeleteAutoSelectProfile(h.App.Database); err != nil {
		return h.RespondWithError(c, err)
	}
