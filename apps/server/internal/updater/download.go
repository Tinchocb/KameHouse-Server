...
func (u *Updater) decompressZip(archivePath string, folderName string) (dest string, err error) {
	topFolderName := "KameHouse-" + u.LatestRelease.Version
	if folderName != "" {
		topFolderName = folderName
	}
	// "/KameHouse-repo/KameHouse-v1.0.0.zip" -> "/KameHouse-repo/KameHouse-1.0.0/"
	dest = filepath.Join(filepath.Dir(archivePath), topFolderName) // "/KameHouse-repo/KameHouse-v1.0.0"
...
//   - "KameHouse-v1.zip" -> "/KameHouse-repo/KameHouse-v1.zip"
