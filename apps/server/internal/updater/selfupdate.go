...
const (
	tempReleaseDir = "KameHouse_new_release"
...
func NewSelfUpdater() *SelfUpdater {
	...
	ret.tmpExecutableName = "kamehouse.exe.old"
	switch runtime.GOOS {
	case "windows":
		ret.tmpExecutableName = "kamehouse.exe.old"
	default:
		ret.tmpExecutableName = "kamehouse.old"
	}
...
func (su *SelfUpdater) recover(assetUrl string) {
	...
	su.logger.Error().Msg("selfupdate: Failed to install update. Update downloaded to 'KameHouse_new_release'")
}

func getExePath() string {
	exe, err := os.Executable() // /path/to/KameHouse.exe
	if err != nil {
		return ""
	}
	exePath, err := filepath.EvalSymlinks(exe) // /path/to/KameHouse.exe
	if err != nil {
		return ""
	}
...
func (su *SelfUpdater) Run() error {
	...
	// Download the asset to exeDir/KameHouse_tmp
	...
	// KameHouse.exe + /backup_restore_if_failed/KameHouse.exe
	...
	// KameHouse.exe -> KameHouse.exe.old
	// LICENSE -> LICENSE.old
	for _, file := range files {
		...
		if err != nil && (file == "kamehouse" || file == "kamehouse.exe") {
			renamingFailed = true
	...
	// Remove KameHouse.exe.old and LICENSE.old
	for _, file := range files {
