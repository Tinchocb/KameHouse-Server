...
import (
	...
	hibikecustomsource "kamehouse/internal/extension/hibike/customsource"
...
func (r *Repository) loadPlugin(ext *extension.Extension) (err error) {
	// Goja plugin runtime removed - plugins are no longer supported
	r.logger.Warn().Str("id", ext.ID).Msg("extensions: Plugin loading is no longer supported (Goja removed)")
	return fmt.Errorf("plugin loading is no longer supported")
}
