...
func NewReadSeeker(t *torrent.Torrent, file *torrent.File, logger ...*zerolog.Logger) io.ReadSeekCloser {
	tr := file.NewReader()
	tr.SetResponsive()
	// Read ahead 20MB for smoother streaming and fewer stalls on seek
	tr.SetReadahead(20 * 1024 * 1024)
...
// PrioritizeDownloadPieces sets high priority for the first 5% and last 5% of pieces.
// Head pieces allow the player to read container headers (MP4 moov, MKV SeekHead) fast.
// Tail pieces ensure the moov atom (if placed at end) and MKV Cues are available quickly.
func PrioritizeDownloadPieces(t *torrent.Torrent, file *torrent.File, logger *zerolog.Logger) {
	...
	filePieceCount := endPieceIdx - firstPieceIdx + 1

	// Head: 5% of the file's pieces, minimum 10
	numPiecesForStart := filePieceCount * 5 / 100
	if numPiecesForStart < 10 {
		numPiecesForStart = 10
	}
	if numPiecesForStart > filePieceCount {
		numPiecesForStart = filePieceCount
	}
	if logger != nil {
		logger.Debug().Msgf("torrentutil: Head priority (5%%) — pieces %d to %d (%d pieces)",
			firstPieceIdx, firstPieceIdx+numPiecesForStart-1, numPiecesForStart)
	}
	...
	// Tail: 5% of the file's pieces, minimum 10
	numPiecesForEnd := filePieceCount * 5 / 100
	if numPiecesForEnd < 10 {
		numPiecesForEnd = 10
	}
	if numPiecesForEnd > filePieceCount {
		numPiecesForEnd = filePieceCount
	}
	tailStart := endPieceIdx - numPiecesForEnd + 1
	if tailStart < firstPieceIdx {
		tailStart = firstPieceIdx
	}
	if logger != nil {
		logger.Debug().Msgf("torrentutil: Tail priority (5%%) — pieces %d to %d (%d pieces)",
			tailStart, endPieceIdx, numPiecesForEnd)
	}
	for idx := tailStart; idx <= endPieceIdx; idx++ {
		if idx >= 0 && int(idx) < t.NumPieces() {
			t.Piece(int(idx)).SetPriority(torrent.PiecePriorityNow)
		}
	}
...
func PrioritizeRangeRequestPieces(rangeHeader string, t *torrent.Torrent, file *torrent.File, logger *zerolog.Logger) {
	...
	if start >= 0 {
		...
		numPiecesToPrioritize := int64(15) // Prioritize next 15 pieces for faster seek recovery
