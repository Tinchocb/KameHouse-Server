...
type (
	FileTreeNode struct {
		...
		LocalFile      mo.Option[*dto.LocalFile]
		MediaIds       []int
		LocalFiles     []*dto.LocalFile // For directory nodes
	...
	FileTreeJSON struct {
		...
		LocalFiles map[string]*dto.LocalFile `json:"localFiles"`
	}
)
...
func (l *LibraryExplorer) buildTreeFromFilePaths(rootNode *FileTreeNode, filePaths []string) {
	...
	for _, libraryPath := range l.libraryPaths {
		libraryNode := &FileTreeNode{
			...
			Children:       make([]*FileTreeNode, 0),
			MediaIds:       make([]int, 0),
			LocalFile:      mo.None[*dto.LocalFile](),
			parent:         rootNode,
		}
	...
	for parentDir, files := range dirToFiles {
		...
		for _, filePath := range files {
			fileNode := &FileTreeNode{
				...
				Children:       make([]*FileTreeNode, 0),
				MediaIds:       make([]int, 0),
				LocalFile:      mo.None[*dto.LocalFile](), // Will be hydrated separately
				parent:         parentNode,
			}
...
func (l *LibraryExplorer) hydrateLocalFileData(tree *FileTree) (map[string]*dto.LocalFile, error) {
...
func (l *LibraryExplorer) hydrateNode(node *FileTreeNode, localFileMap map[string]*dto.LocalFile) {
	...
	localFileSet := make(map[string]*dto.LocalFile)
	...
	if node.Kind == FileNode {
		...
		if localFile, exists := localFileMap[node.NormalizedPath]; exists {
			...
			node.LocalFile = mo.None[*dto.LocalFile]()
		}
	} else {
