...
import (
	...
	hibikeonlinestream "kamehouse/internal/extension/hibike/onlinestream"
	onlinestream_providers "kamehouse/internal/onlinestream/providers"
...
func (r *Repository) getEpisodeContainer(provider string, media *anilist.BaseAnime, from int, to int, dubbed bool, year int) (*episodeContainer, error) {
	...
	for _, episodeDetails := range providerEpisodeList {
		...
		if episodeDetails.Number >= from && episodeDetails.Number <= to {
			...
			// Proactive N+1 Caching
			go func(epNumber int, details *hibikeonlinestream.EpisodeDetails) {

				isBingeWatching := true
				if r.continuityManager != nil && r.continuityManager.Get() != nil {
					// Hacky reflection/interface casting since we use any ref
					if mgr, ok := r.continuityManager.Get().(interface {
						CalculateBingeProbability(mediaId int) struct {
							MediaId        int
							Probability    float64
							IsBingeCooling bool
						}
					}); ok {
						prob := mgr.CalculateBingeProbability(media.ID)
						if prob.Probability < 0.70 {
				...
				if !isBingeWatching {
					r.logger.Debug().Msgf("onlinestream: Skipping N+1 cache for episode '%d' as User Binge probability is too low", epNumber)
					return
				}

				nextNum := epNumber + 1
				var nextDetails *hibikeonlinestream.EpisodeDetails
				for _, pd := range providerEpisodeList {
					if pd.Number == nextNum {
						nextDetails = pd
						break
					}
				}
				...
				if nextDetails != nil {
					nextKey := fmt.Sprintf("%d$%s$%d$%v", media.ID, provider, nextDetails.Number, dubbed)
					var nextCached *episodeData
					if found, _ := r.fileCacher.Get(fcEpisodeDataBucket, nextKey, &nextCached); !found {
						r.logger.Debug().
							Str("key", nextKey).
							Msgf("onlinestream: Proactively fetching next episode '%d' servers", nextDetails.Number)

						// Zoro dub checks
						if provider == onlinestream_providers.ZoroProvider && dubbed {
							if strings.HasSuffix(nextDetails.ID, string(hibikeonlinestream.SubAndDub)) {
								nextDetails.ID = strings.Replace(nextDetails.ID, string(hibikeonlinestream.SubAndDub), string(hibikeonlinestream.Dub), 1)
							} else {
								return // Skip sub-only
							}
						}

						nextServers, err := r.getProviderEpisodeServers(provider, nextDetails)
						if err == nil {
							nextEpisode := &episodeData{
								ID:      nextDetails.ID,
								Number:  nextDetails.Number,
								Title:   nextDetails.Title,
								Servers: nextServers,
							}
							_ = r.fileCacher.Set(fcEpisodeDataBucket, nextKey, nextEpisode)
						}
					}
				}
			}(episodeDetails.Number, episodeDetails)
