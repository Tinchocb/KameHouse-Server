...
func (s *AutoSelect) filterAndSort(torrents []*hibiketorrent.AnimeTorrent, profile *dto.AutoSelectProfile, postSearchSort func([]*hibiketorrent.AnimeTorrent) []*TorrentWithCacheStatus) []*hibiketorrent.AnimeTorrent {
...
func (s *AutoSelect) filter(torrents []*hibiketorrent.AnimeTorrent, profile *dto.AutoSelectProfile) []*hibiketorrent.AnimeTorrent {
	candidates := make([]*candidate, len(torrents))
	for i, t := range torrents {
		candidates[i] = &candidate{
			torrent:   t,
...
func (s *AutoSelect) sort(torrents []*hibiketorrent.AnimeTorrent, profile *dto.AutoSelectProfile) {
	candidates := make([]*candidate, len(torrents))
	for i, t := range torrents {
		candidates[i] = &candidate{
			torrent:   t,
...
func checkPreference(condition bool, preference dto.AutoSelectPreference) bool {
	if preference == dto.AutoSelectPreferenceOnly && !condition {
		return false
	}
	if preference == dto.AutoSelectPreferenceNever && condition {
		return false
	}
	return true
}

func (s *AutoSelect) filterCandidates(candidates []*candidate, profile *dto.AutoSelectProfile) []*candidate {
	if profile == nil {
		return candidates
	}
	...
	for _, c := range candidates {
		...
		// bad
		if profile.BestReleasePreference == dto.AutoSelectPreferenceOnly && !t.IsBestRelease {
			continue
		}

		if profile.BestReleasePreference == dto.AutoSelectPreferenceNever && t.IsBestRelease {
			continue
		}
...
func (s *AutoSelect) sortCandidates(candidates []*candidate, profile *dto.AutoSelectProfile) {
	for _, c := range candidates {
...
func (s *AutoSelect) calculateScore(c *candidate, profile *dto.AutoSelectProfile) int {
	score := 0
	parsed := c.parsed
	t := c.torrent
	...
	//if (profile.BestReleasePreference != dto.AutoSelectPreferenceAvoid && profile.BestReleasePreference != dto.AutoSelectPreferenceNever) &&
	...
	if profile.MultipleAudioPreference == dto.AutoSelectPreferencePrefer && isMultiAudio {
		score += scoreMultiAudio
	}
	if profile.MultipleAudioPreference == dto.AutoSelectPreferenceAvoid && isMultiAudio {
		score -= scoreMultiAudio
	}

	// Multiple subs preference (prefer/avoid)
	isMultiSubs := containsMultiOrDual(parsed.Subtitles)
	if profile.MultipleSubsPreference == dto.AutoSelectPreferencePrefer && isMultiSubs {
		score += scoreMultiSubs
	}
	if profile.MultipleSubsPreference == dto.AutoSelectPreferenceAvoid && isMultiSubs {
		score -= scoreMultiSubs
	}

	// Batch preference (prefer/avoid)
	isBatch := t.IsBatch
	if profile.BatchPreference == dto.AutoSelectPreferencePrefer && isBatch {
		score += scoreBatch
	}
	if profile.BatchPreference == dto.AutoSelectPreferenceAvoid && isBatch {
		score -= scoreBatch
	}

	// Best release preference (prefer/avoid)
	isBestRelease := t.IsBestRelease && (t.Seeders == -1 || t.Seeders > 2)
	if profile.BestReleasePreference == dto.AutoSelectPreferencePrefer && isBestRelease {
		score += scoreBestRelease
	}
	if profile.BestReleasePreference == dto.AutoSelectPreferenceAvoid && isBestRelease {
		score -= scoreBestRelease
	}
