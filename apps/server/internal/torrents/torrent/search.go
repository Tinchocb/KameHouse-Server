	...
	if opts.animeMetadata.IsPresent() {
		...
		if isProbablySameEpisode(parsedData.EpisodeNumber, opts.searchOpts.EpisodeNumber, opts.animeMetadata.MustGet().GetOffset()) {
			...
			episode = &anime.Episode{
				...
				LocalFile:             nil,
				IsDownloaded:          false,
				EpisodeMetadata:       anime.NewEpisodeMetadata(amw, nil, strconv.Itoa(opts.searchOpts.EpisodeNumber), models.ToLibraryMedia(opts.media)),
				FileMetadata:          nil,
				IsInvalid:             false,
				MetadataIssue:         "",
				BaseAnime:             models.ToLibraryMedia(opts.media),
			}
		}
